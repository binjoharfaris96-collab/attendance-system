...
export function StreamForm({ buildingId }: { buildingId: string | null }) {
  ...
  const handleCapture = (type: string, capture?: string) => {
    // Check if we want to trigger the custom webcam modal
  ...
  const handleWebcamCapture = (blob: Blob, type: "photo" | "video") => {
    // In a real app, you would upload the blob to a storage service (S3, Vercel Blob)
    // For this prototype, we simulate it by creating a local object URL
    const url = URL.createObjectURL(blob);
    setAttachmentUrl(url);
    setAttachmentName(`captured-${type}-${new Date().getTime()}.${type === "photo" ? "jpg" : "webm"}`);
  };
                  ...
                  <div className="flex items-center justify-between p-2 rounded-lg bg-emerald-500/5 border border-emerald-500/20">
                     <div className="flex items-center gap-2">
                        {attachmentUrl.startsWith("blob:") ? (
                          webcamMode === "photo" ? <ImageIcon className="w-4 h-4 text-emerald-500" /> : <Video className="w-4 h-4 text-emerald-500" />
                        ) : (
