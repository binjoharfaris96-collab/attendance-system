...
export default async function BuildingsPage() {
  ...
  return (
    <div className="p-8 max-w-4xl mx-auto">
      ...
      <section className="bg-white rounded-2xl shadow-xl shadow-slate-200/60 border border-slate-100 p-8 mb-10 overflow-hidden relative">
        ...
        <form action={handleAddBuilding} className="grid grid-cols-1 md:grid-cols-2 gap-6 relative">
          <div className="flex flex-col gap-2">
            <label className="text-sm font-semibold text-slate-600 ml-1">Building Name</label>
            <input
              name="name"
              placeholder="e.g., East Wing Architecture"
              className="px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-4 focus:ring-indigo-100 focus:border-indigo-400 outline-none transition-all duration-200 placeholder:text-slate-400"
              required
            />
          </div>
          <div className="flex flex-col gap-2">
            <label className="text-sm font-semibold text-slate-600 ml-1">Address (Optional)</label>
            <input
              name="address"
          ...
          <div className="md:col-span-2 mt-2">
            <button
              type="submit"
              className="w-full md:w-auto px-10 py-3.5 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-xl shadow-lg shadow-indigo-200 transition-all duration-200 active:scale-95 flex items-center justify-center gap-2"
            >
      ...
      <section className="bg-white rounded-2xl shadow-xl shadow-slate-200/60 border border-slate-100 p-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {buildings.map((b) => (
            <div key={b.id} className="group p-5 bg-white border border-slate-100 rounded-2xl hover:border-indigo-200 hover:shadow-lg hover:shadow-slate-100 transition-all duration-300 flex justify-between items-center">
              <div className="flex items-center gap-4">
                ...
                <div className="flex flex-col min-w-0">
                  ...
                  <p className="text-sm text-slate-500 truncate">{b.address || "No address provided"}</p>
                </div>
              </div>
          ...
          {buildings.length === 0 && (
            <div className="col-span-full py-12 text-center bg-slate-50 rounded-2xl border border-dashed border-slate-200">
              <p className="text-slate-400 font-medium italic">No buildings registered yet.</p>
            </div>
          )}
        </div>
      </section>
